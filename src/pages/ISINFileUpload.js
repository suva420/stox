import { useCallback } from "react";
import "antd/dist/antd.min.css";
import { Button } from "@mui/material";
import {
  Menu as AntMenu,
  Dropdown as AntDropdown,
  Button as AntButton,
} from "antd";
import {
  DownOutlined,
  ArrowLeftOutlined,
  ArrowRightOutlined,
  CalendarOutlined,
  CheckOutlined,
  ClockCircleOutlined,
  CloseOutlined,
  DeleteOutlined,
  EditOutlined,
  ExclamationCircleOutlined,
  HeartOutlined,
  LeftOutlined,
  LockOutlined,
  MailOutlined,
  PaperClipOutlined,
  PhoneOutlined,
  QuestionCircleOutlined,
  ReloadOutlined,
  RightOutlined,
  SearchOutlined,
  SendOutlined,
  ShareAltOutlined,
  UserOutlined,
} from "@ant-design/icons";
import { useNavigate } from "react-router-dom";
import "./ISINFileUpload.css";

const ISINFileUpload = () => {
  const navigate = useNavigate();

  const onButtonClick = useCallback(() => {
    navigate("/");
  }, [navigate]);

  return (
    <div className="isin-file-upload">
      <div className="graph5">
        <img className="vector-icon20" alt="" src="../vector5.svg" />
        <img className="frame-icon1" alt="" src="../frame.svg" />
        <Button
          className="cta12"
          sx={{ width: 129 }}
          variant="contained"
          color="primary"
        >
          Save
        </Button>
        <Button
          className="cta13"
          sx={{ width: 129 }}
          variant="contained"
          color="primary"
        >
          Cancel
        </Button>
        <div className="isin-file-updation">ISIN File Updation</div>
        <div className="upload">
          <div className="top">
            <div className="back">
              <img className="icon" alt="" src="../icon.svg" />
            </div>
            <div className="title">Upload File</div>
          </div>
          <div className="container2">
            <div className="content2">
              <img className="icon1" alt="" src="../icon1.svg" />
              <div className="paragraph">
                Browse and chose the files you want to upload from your computer
              </div>
              <input className="cta14" type="file" />
            </div>
          </div>
        </div>
      </div>
      <div className="navbar6">
        <div className="frame-div27">
          <img className="stock-1-icon6" alt="" src="../stock-1@2x.png" />
          <b className="stoxx6">Stoxx</b>
        </div>
        <div className="right8">
          <div className="buttons6">
            <Button
              className="button7"
              variant="outlined"
              color="primary"
              href="/home"
              onClick={onButtonClick}
            >
              LOGOUT
            </Button>
          </div>
        </div>
        <AntDropdown
          className="help-dropdown7"
          overlay={
            <AntMenu>
              {[{ value: "FAQ" }, { value: "Guide" }].map((option, index) => (
                <AntMenu.Item key={index}>
                  <a onClick={(e) => e.preventDefault()}>
                    {option.value || ""}
                  </a>
                </AntMenu.Item>
              ))}
            </AntMenu>
          }
          placement="bottomLeft"
          trigger={["hover"]}
        >
          <AntButton onClick={(e) => e.preventDefault()}>
            {`Help `}
            <DownOutlined />
          </AntButton>
        </AntDropdown>
        <AntDropdown
          className="report-dropdown6"
          overlay={
            <AntMenu>
              {[
                { value: "Benpos Master Query" },
                { value: "Movement Of Shares" },
                { value: "Bought/Sold Shares" },
                { value: "PAN Listed Share Hold" },
              ].map((option, index) => (
                <AntMenu.Item key={index}>
                  <a onClick={(e) => e.preventDefault()}>
                    {option.value || ""}
                  </a>
                </AntMenu.Item>
              ))}
            </AntMenu>
          }
          placement="bottomLeft"
          trigger={["hover"]}
        >
          <AntButton onClick={(e) => e.preventDefault()}>
            {`Reports `}
            <DownOutlined />
          </AntButton>
        </AntDropdown>
        <AntDropdown
          className="upload-dropdown6"
          overlay={
            <AntMenu>
              {[
                { value: "NSDL" },
                { value: "CSDL" },
                { value: "PAN Master File" },
                { value: "ISIN Master File" },
              ].map((option, index) => (
                <AntMenu.Item key={index}>
                  <a onClick={(e) => e.preventDefault()}>
                    {option.value || ""}
                  </a>
                </AntMenu.Item>
              ))}
            </AntMenu>
          }
          placement="bottomLeft"
          trigger={["hover"]}
        >
          <AntButton onClick={(e) => e.preventDefault()}>
            {`Upload `}
            <DownOutlined />
          </AntButton>
        </AntDropdown>
        <AntDropdown
          className="file-dropdown6"
          overlay={
            <AntMenu>
              {[{ value: "Company Master Details" }, { value: "Logout" }].map(
                (option, index) => (
                  <AntMenu.Item key={index}>
                    <a onClick={(e) => e.preventDefault()}>
                      {option.value || ""}
                    </a>
                  </AntMenu.Item>
                )
              )}
            </AntMenu>
          }
          placement="bottomLeft"
          trigger={["hover"]}
        >
          <AntButton onClick={(e) => e.preventDefault()}>
            {`File `}
            <DownOutlined />
          </AntButton>
        </AntDropdown>
      </div>
    </div>
  );
};

export default ISINFileUpload;
